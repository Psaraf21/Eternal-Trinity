...
import "../css/TemplePage.css"
const TemplePage = ({ searchQuery }) => {
  const templeData = [
    {
      id: "rammandir",
      imageUrl: ramMandir,
  ...
  // Add a check to ensure templeData is defined before filtering
  const filteredTempleData = templeData
  ? templeData.filter((temple) =>
      temple.id.toLowerCase().includes((searchQuery || '').toLowerCase())
    )
  : [];


  // Render a default message if no results are found
  const renderNoResultsMessage = () => {
    if (filteredTempleData.length === 0 && searchQuery.trim() !== '') {
  ...
  return (
    <div className='Temple_Info_Page'>
      <Header/>
      {/* <h2 >Search Results</h2> */}
      <div className='title_Events_Update'><h1>Search Results</h1></div>
      {/* <h2>hdhd</h2> */}
      <div className='Temple_page_main'>

      <div className="TEMPLE_PAGE">
        {filteredTempleData.map((temple) => (
          <div key={temple.id} className="">
            <img src={temple.imageUrl} alt="" />
            <div className="Temple_Info">
              <h3>{temple.title}</h3>
      ...
      <footer className="footer_Temple_page">
      <div className="container_Temple_page">
        <p className="copyright_Temple_page">© {new Date().getFullYear()} Temple Name. All Rights Reserved.</p>
        <nav>
